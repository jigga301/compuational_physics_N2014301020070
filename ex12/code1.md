...
for i in range(151):
    ...
    for j in range(151):
        if i == 0 or i == 150 or j == 0 or j == 150:
            voltage = 0
        elif 50 <= i <= 90 and j == 50:
            voltage = 1
        elif 50 <= i <= 90 and j == 90:
            voltage = -1
        else:
            voltage = 0
...
plt.colorbar(CS)
plt.title('electric potential near two capacitor plates')
plt.xlabel('x(m)')
plt.ylabel('y(m)')

fig = figure()
ax = Axes3D(fig)
surf = ax.plot_surface(X, Y, Z, rstride=1, cstride=1, cmap=cm.hot, linewidth=0, antialiased=False)
ax.set_xlabel('x(m)')
ax.set_ylabel('y(m)')
ax.set_zlabel('voltage(V)')
ax.set_title('voltage in 3D picture')
fig.colorbar(surf, shrink=0.5, aspect=5)

fig0, ax0 = plt.subplots()
strm = ax0.streamplot(X, Y, np.array(Ey), np.array(Ex), color=np.array(E), linewidth=2, cmap=plt.cm.coolwarm)
