...
class elliptical_orbit(object):
    ...
    def calculate(self):
        ...
        while (self.t[-1] < 1):
            ...
            self.vx.append(self.vx[-1] - 4 * (np.pi ** 2) * self.x[-1] / (temp_r ** 3) * self.dt)
            self.vy.append(self.vy[-1] - 4 * (np.pi ** 2) * self.y[-1] / (temp_r ** 3) * self.dt)
            self.x.append(self.x[-1] + self.vx[-1] * self.dt)
            self.y.append(self.y[-1] + self.vy[-1] * self.dt)
            self.t.append(self.t[-1] + self.dt)
            if (self.x[-1] > 0 and self.y[-1] >= 0 and self.y[-2] <= 0 and self.t[-2] > 0):
